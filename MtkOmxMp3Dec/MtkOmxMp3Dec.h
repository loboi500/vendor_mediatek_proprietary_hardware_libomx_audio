...
private:
    ...
    bool GetMPEGAudioFrameSize(
        ...
        int *out_bitrate = NULL, unsigned *out_layer = NULL, int *out_num_samples = NULL);
    ...
    void handleFormatChanged(OMX_U8* pBitStreamRead /*input mp3 frame*/,
                                    int sample_rate, int num_channels);
    bool processFrameAlignment(OMX_BUFFERHEADERTYPE *pInputBuf,
            OMX_BUFFERHEADERTYPE *pOutputBuf);
    OMX_U32 formatHeader(OMX_BUFFERHEADERTYPE *pInputBuf);
    ...
    OMX_U8 *mInputCachedBuffer;
    OMX_U32 mInputCachedBufferFilledLength;
};
