...
#if ANDROID
    prctl(PR_SET_NAME, "MtkOmxAudioDec", 0, 0, 0);
#endif
/*
...
*/
	...
	LOGV("Audio Decode Thread priority is %d", getpriority(PRIO_PROCESS, 0));

    LOGV("MtkOmxAudioDecodeThread created pOmxAudioDec=%p, tid=%d", pOmxAudioDec, gettid());
    ...
    SIGNAL(pOmxAudioDec->mDecodeSem);
    LOGV("MtkOmxAudioDecodeThread terminated pOmxAudioDec=%p", pOmxAudioDec);
    return NULL;
}
...
#if ANDROID
    prctl(PR_SET_NAME, "MtkOmxAudioCmd", 0, 0, 0);
#endif
...
*/
	...
	LOGV("Audio Command Thread priority is %d", getpriority(PRIO_PROCESS, 0));

    LOGV("MtkOmxAudioCommandThread created pOmxAudioDec=%p", pOmxAudioDec);
    ...
    while (1)
    {
        ...
        if (-1 == status)
        {
            LOGE("[%p] poll error %d (%s), fd:%d", pOmxAudioDec, errno, strerror(errno), pOmxAudioDec->mCmdPipe[MTK_OMX_PIPE_ID_READ]);
            LOGE("[%p] pipe: %d %d", pOmxAudioDec, pOmxAudioDec->mCmdPipe[MTK_OMX_PIPE_ID_READ], pOmxAudioDec->mCmdPipe[MTK_OMX_PIPE_ID_WRITE]);
        ...
        else
        {
            ...
            {
                ALOGE("[0x%p] FD: %d %d, Poll get unsupported event:0x%x", pOmxAudioDec, pOmxAudioDec->mCmdPipe[0], pOmxAudioDec->mCmdPipe[1], PollFd.revents);
                abort();
            }
        }
...
MtkOmxAudioDecBase::MtkOmxAudioDecBase()
{
    ...
    mlog_enable = __android_log_is_loggable(ANDROID_LOG_DEBUG,"MtkOmxAudioDecBase_LOG",
            ANDROID_LOG_INFO);
#endif
    LOGD_IF(mlog_enable, "MtkOmxAudioDecBase::MtkOmxAudioDecBase this= %p", this);
        ...
        OMX_IN OMX_STRING componentName)
{
    ...
    if (mFileDumpCtrl == 0) {
        mFileDumpCtrl = DATA_DUMP_CTL; //use #define value when property is empty
    }
        ...
        OMX_IN OMX_U32 nSizeBytes)
{
    ...
    if (nPortIndex == mInputPortDef.nPortIndex)
    {
        ...
        *ppBufferHdr = mInputBufferHdrs[mInputBufferPopulatedCnt] = (OMX_BUFFERHEADERTYPE *)MTK_OMX_ALLOC(sizeof(OMX_BUFFERHEADERTYPE));
        if (*ppBufferHdr == NULL) {
            LOGE("AllocateBuffer:Fail to Allocate Memory for input BufferHeader: %d", mInputBufferPopulatedCnt);
            err = OMX_ErrorInsufficientResources;
            goto EXIT;
        }
        (*ppBufferHdr)->pBuffer = (OMX_U8 *)MTK_OMX_ALLOC(nSizeBytes); // allocate input from dram
        if ((*ppBufferHdr)->pBuffer == NULL) {
            LOGE("AllocateBuffer:Fail to Allocate Memory for input Buffer: %d", mInputBufferPopulatedCnt);
            MTK_OMX_FREE(*ppBufferHdr);
            *ppBufferHdr = NULL;
            err = OMX_ErrorInsufficientResources;
            goto EXIT;
        }
    ...
    else if (nPortIndex == mOutputPortDef.nPortIndex)
    {
        ...
        *ppBufferHdr = mOutputBufferHdrs[mOutputBufferPopulatedCnt] = (OMX_BUFFERHEADERTYPE *)MTK_OMX_ALLOC(sizeof(OMX_BUFFERHEADERTYPE));
        if (*ppBufferHdr == NULL) {
            LOGE("AllocateBuffer:Fail to Allocate Memory for output BufferHeader: %d", mOutputBufferPopulatedCnt);
            err = OMX_ErrorInsufficientResources;
            goto EXIT;
        }
        (*ppBufferHdr)->pBuffer = (OMX_U8 *)MTK_OMX_ALLOC(nSizeBytes); // allocate output from dram
        if ((*ppBufferHdr)->pBuffer == NULL) {
            LOGE("AllocateBuffer:Fail to Allocate Memory for output Buffer: %d", mOutputBufferPopulatedCnt);
            MTK_OMX_FREE(*ppBufferHdr);
            *ppBufferHdr = NULL;
            err = OMX_ErrorInsufficientResources;
            goto EXIT;
        ...
        OMX_IN OMX_U8 *pBuffer)
{
    ...
    if (nPortIndex == mInputPortDef.nPortIndex)
    {
        ...
        *ppBufferHdr = mInputBufferHdrs[mInputBufferPopulatedCnt] = (OMX_BUFFERHEADERTYPE *)MTK_OMX_ALLOC(sizeof(OMX_BUFFERHEADERTYPE));
        ...
        LOGV("UseBuffer port_idx(0x%X), idx[%d], pBuffHead(0x%08X), pBuffer(%p)", (unsigned int)nPortIndex, (int)mInputBufferPopulatedCnt, (unsigned int)mInputBufferHdrs[mInputBufferPopulatedCnt], pBuffer);
        ...
        OMX_IN OMX_BUFFERHEADERTYPE *pBuffHead)
{
    ...
    if ((nPortIndex == MTK_OMX_INPUT_PORT) && bAllowFreeBuffer)
    {
        ...
        for (OMX_U32 i = 0 ; i < mInputPortDef.nBufferCountActual ; i++)
        {
            ...
            {
                LOGD_IF(mlog_enable, "FreeBuffer input (%p)", pBuffHead);
                MTK_OMX_FREE(mInputBufferHdrs[i]);
                mInputBufferHdrs[i] = NULL;
                mInputBufferPopulatedCnt--;
            }
    ...
    if ((nPortIndex == MTK_OMX_OUTPUT_PORT) && bAllowFreeBuffer)
    {
        ...
        for (OMX_U32 i = 0 ; i < mOutputPortDef.nBufferCountActual ; i++)
        {
            ...
            {
                LOGD_IF(mlog_enable, "FreeBuffer output (%p)", pBuffHead);
                MTK_OMX_FREE(mOutputBufferHdrs[i]);
                mOutputBufferHdrs[i] = NULL;
                mOutputBufferPopulatedCnt--;
            }
...
OMX_ERRORTYPE MtkOmxAudioDecBase::HandleMarkBuffer(OMX_U32 nParam1, OMX_PTR pCmdData)
{
    ...
    if (pCmdData != NULL)
    {
        ...
        LOGV("mpMark->hMarkTargetComponent(0x%p),mpMark->pMarkData(0x%p)", mpMark->hMarkTargetComponent, mpMark->pMarkData);
    }
    return err;
}
...
void MtkOmxAudioDecBase::DumpETBQ()
{
    ...
    for (iter = mEmptyThisBufQ.begin(); iter != mEmptyThisBufQ.end(); iter++)
    {
        LOGD("[%d] - pBuffHead(%p)", *iter, mInputBufferHdrs[mEmptyThisBufQ[i]]);
    }
...
void MtkOmxAudioDecBase::DumpFTBQ()
{
    ...
    for (iter = mFillThisBufQ.begin(); iter != mFillThisBufQ.end(); iter++)
    {
        LOGD("[%d] - pBuffHead(%p)", *iter, mOutputBufferHdrs[mFillThisBufQ[i]]);
    }
    ...
    for (size_t i = 0 ; i < mFillThisBufQ.size() ; i++)
    {
        ...
        LOGV("pBuffHead(%p)", mOutputBufferHdrs[mFillThisBufQ[i]]);
    }
